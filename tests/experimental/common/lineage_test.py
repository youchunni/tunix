...
class LineageTest(absltest.TestCase):
  ...
  def test_lineage_context_merge_deduplicates_parent_ids(self):
    ...
    merged = lineage.LineageContext.merge(
        ...
        component=lineage.Component.ORCHESTRATOR_ASSEMBLER,
        operation=lineage.Operation.PACK,
    )

    self.assertEqual(merged.parent_tracking_ids, ["traj_p1_0", "traj_p2_0"])

  def test_standard_components_and_operations(self):
    self.assertEqual(lineage.Component.ENGINE_DISPATCH, "engine.dispatch")
    self.assertEqual(lineage.Component.WORKER_ROLLOUT, "worker.rollout")
    self.assertEqual(
        lineage.Component.ORCHESTRATOR_ASSEMBLER, "orchestrator.assembler"
    )
    self.assertEqual(lineage.Component.WORKER_TRAINER, "worker.trainer")
    self.assertEqual(lineage.Component.ENGINE_TRAIN, "engine.train")

    self.assertEqual(lineage.Operation.ROLLOUT, "rollout")
    self.assertEqual(lineage.Operation.GENERATE, "generate")
    self.assertEqual(lineage.Operation.PACK, "pack")
    self.assertEqual(lineage.Operation.FWD_BWD, "fwd_bwd")
    self.assertEqual(lineage.Operation.TRAIN_STEP, "train_step")
